# coding: utf-8
from sqlalchemy import CHAR, CheckConstraint, Column, DateTime, ForeignKey, ForeignKeyConstraint, Index, Integer, Unicode, VARCHAR, text
...
class Empresa(Base):
    ...
    ide_empresa_ide = Column(NUMBER(asdecimal=False))
    ...
    nalbaran_notas_pedido = Column(Unicode(1), server_default=text("'N'"))
...
class Direccione(Base):
    ...
    __table_args__ = (
        Index('ind_dire_pobl', 'poblacion', 'estado'),
    )
...
class EmpresasDistancia(Base):
    __tablename__ = 'empresas_distancias'

    id = Column(NUMBER(12, 0, False), primary_key=True)
    ide = Column(ForeignKey('cm_empresas.id'), nullable=False)
    empresa = Column(ForeignKey('empresas.id'), nullable=False)
    codigo_origen = Column(VARCHAR(100), nullable=False)
    codigo_destino = Column(VARCHAR(100), nullable=False)
    distancia = Column(NUMBER(5, 0, False), nullable=False, server_default=text("0                     "))

    empresa1 = relationship('Empresa')
...
class TarifasCliente(Base):
    ...
    id = Column(NUMBER(6, 0, False), primary_key=True)
    cliente = Column(ForeignKey('empresas.idc'))
    ...
    unidad2 = Column(CHAR(1))
    desde2 = Column(NUMBER(10, 4, True))
    hasta2 = Column(NUMBER(10, 4, True))
    minimo2 = Column(NUMBER(10, 4, True))
    maximo2 = Column(NUMBER(10, 4, True))
...
class EmpresasDireccione(Base):
    ...
    empresa = Column(ForeignKey('empresas.id'), primary_key=True, nullable=False)
    direccion = Column(ForeignKey('direcciones.id'), primary_key=True, nullable=False)
    tipo = Column(CHAR(1))
...
class Pedido(Base):
    ...
    __table_args__ = (
        ...
        Index('ind_pedi_fechacre', 'delegacion', 'estado', 'ide'),
...
class PedidosEtapa(Base):
    ...
    __table_args__ = (
        ...
        Index('ind_peet_idotlin', 'id_ot_linea', 'ide'),
